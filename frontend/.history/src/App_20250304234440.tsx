...
// // export default App;
import { Route, Routes, useNavigate } from 'react-router-dom'; // ❌ 不要导入 BrowserRouter
import { AppBar, Toolbar, Link, Button, Box } from '@mui/material';
import { logout, getUserRole } from './services/authApi';
import PrivateRoute from './components/PrivateRoute';
import AnimalList from './pages/AnimalList';
...
function App() {
  const navigate = useNavigate(); // ✅ 现在 useNavigate() 不会报错
  const userRole = getUserRole();

  const handleLogout = () => {
    logout();
    navigate('/login'); // ✅ 使用 navigate() 代替 window.location.href
  };
