...
function App() {
  // 根据 localStorage 中的 isAdmin 判断当前用户是否为管理员
    ...
    <Router>
      <AppBar position="static">
        <Toolbar>
          ...
          {/* 仅当 isAdmin 为 true 时显示管理员入口 */}
          {isAdmin && (
            <>
              <Link component={RouterLink} to="/admin-event" color="inherit" underline="none" sx={{ mr: 2 }}>
                Publish Event
              </Link>
              <Link component={RouterLink} to="/admin/registrations/1" color="inherit" underline="none" sx={{ mr: 2 }}>
      ...
      <Routes>
        ...
        <Route path="/task-list" element={<PrivateRoute><TaskList /></PrivateRoute>} />
        {/* 管理员专用页面 */}
        <Route path="/admin-event" element={<PrivateRoute>{isAdmin ? <AdminEventForm /> : <div>Access Denied</div>}</PrivateRoute>} />
        {/* 这里的 eventId 为动态参数，访问时地址为 /admin/registrations/2 等 */}
        <Route path="/admin/registrations/:eventId" element={<PrivateRoute>{isAdmin ? <AdminRegistrations /> : <div>Access Denied</div>}</PrivateRoute>} />
      </Routes>
    </Router>
  );
}
