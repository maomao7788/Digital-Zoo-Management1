...
const EventManagement: React.FC = () => {
  ...
  const fetchRegistrations = () => {
    axios.get("http://localhost:8000/zoo/api/registrations/")
      .then(response => {
        setRegistrations(response.data);
      })
      .catch(error => console.error("Error fetching registrations:", error));
  ...
  const filteredRegistrations = selectedEvent
    ? registrations.filter(reg => 
        (typeof reg.event === 'number' ? reg.event : reg.event.id) === selectedEvent.id
      )
    : [];
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 4, p: 2, border: '1px solid #ccc' }}>
        ...
        <form onSubmit={handleSubmit}>
          <TextField label="Title" name="title" value={formData.title} 
            onChange={(e) => setFormData({...formData, title: e.target.value})} fullWidth sx={{ mb: 2 }} required/>
          <TextField label="Description" name="description" value={formData.description} 
            onChange={(e) => setFormData({...formData, description: e.target.value})} fullWidth multiline sx={{ mb: 2 }} required/>
          <TextField label="Event Date" name="event_date" type="datetime-local" value={formData.event_date ? formData.event_date.slice(0,16) : ''}
            onChange={(e) => setFormData({...formData, event_date: e.target.value})} fullWidth sx={{ mb: 2 }} required/>
          <TextField label="Location" name="location" value={formData.location}
            onChange={(e) => setFormData({...formData, location: e.target.value})} fullWidth sx={{ mb: 2 }} required/>
          <FormControl fullWidth sx={{ mb: 2 }}>
            <InputLabel id="min-tier-label">Minimum Tier</InputLabel>
            <Select labelId="min-tier-label" name="min_tier" value={formData.min_tier || ''} 
              label="Minimum Tier" onChange={(e) => setFormData({...formData, min_tier: e.target.value})} required>
              <MenuItem value="Basic">Basic</MenuItem>
              <MenuItem value="Premium">Premium</MenuItem>
              <MenuItem value="VIP">VIP</MenuItem>
            </Select>
          </FormControl>
          <Button variant="contained" type="submit">{selectedEvent ? "Update Event" : "Create Event"}</Button>
          {selectedEvent && (
            <Button variant="outlined" onClick={() => { setSelectedEvent(null); setFormData({ title: '', description: '', event_date: '', location: '', min_tier: '' }); }}
              sx={{ ml: 2 }}>
              Cancel
            </Button>
          )}
      ...
      {selectedEvent && filteredRegistrations.map(reg => (
        <Card key={reg.id} sx={{ mb: 2 }}>
          <CardContent>
            <Typography>User: {reg.user.username}</Typography>
            <Typography>Status: {reg.approved ? "Approved " : "Rejected "}</Typography>
            <Button variant="outlined" onClick={() => toggleRegistrationStatus(reg)}>
              {reg.approved ? "Reject " : "Approve "}
            </Button>
          </CardContent>
        </Card>
      ))}
