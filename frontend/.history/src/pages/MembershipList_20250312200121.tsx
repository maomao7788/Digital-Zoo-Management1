...
const MembershipList = () => {
    ...
    useEffect(() => {
        axios.get("/api/membership/list/", { withCredentials: true })
            .then(res => setMemberships(res.data))
            .catch(err => console.error("Error fetching memberships:", err));
    }, []);

    const activateMembership = async (id: number) => {
        try {
            await axios.put(`/api/membership/${id}/activate/`, {}, { withCredentials: true });
                ...
                <tbody>
                    {memberships.map(m => (
                        <tr key={m.id} className="border">
                            ...
                            <td className="p-2">
                                {!m.is_active && (
                                    <button onClick={() => activateMembership(m.id)} className="p-2 bg-blue-500 text-white rounded">
                                        Activate
                                    </button>
                                )}
                            </td>
