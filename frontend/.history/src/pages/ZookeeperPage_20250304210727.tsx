...
const ZookeeperPage: React.FC = () => {
  ...
  {zookeepers.map(zookeeper => (
    <TableRow key={zookeeper.id}>
      <TableCell>{zookeeper.user_info?.username || "Unknown"}</TableCell>  // ✅ 显示 username
      <TableCell>{zookeeper.user_info?.email || "Unknown"}</TableCell>  // ✅ 显示 email
      ...
      <TableCell>
        ...
        <Button variant="contained" color="error" onClick={() => handleDeleteZookeeper(zookeeper.id!)} style={{ marginLeft: 10 }}>Delete</Button>
      </TableCell>
    </TableRow>
  ))}
</TableBody>
