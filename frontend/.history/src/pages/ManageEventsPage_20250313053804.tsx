...
const EventManagement: React.FC = () => {
  ...
  const fetchRegistrations = () => {
    axios.get("http://localhost:8000/zoo/api/registrations/")
      .then(response => {
        setRegistrations(response.data);
        // 调试时可打印注册信息，查看 reg.event 字段的实际结构
  ...
  const updateRegistrationStatus = async (reg: Registration, newApproved: boolean) => {
    try {
      await axios.post(
        `http://localhost:8000/zoo/api/registrations/${reg.id}/update_status/`,
+      { approved: newApproved },
  ...
  const filteredRegistrations = selectedEvent
    ? registrations.filter(reg => {
        ...
        if (typeof reg.event === 'number') {
          return reg.event === selectedEvent.id;
        }
        if (reg.event && typeof reg.event === 'object' && 'id' in reg.event) {
          return reg.event.id === selectedEvent.id;
        }
        return false;
      })
    : [];
  ...
  return (
    <Box sx={{ p: 3 }}>
     ...
     <Typography>Approved: {reg.approved ? "Yes" : "No"}</Typography>


      {!reg.approved && (
        <FormControl fullWidth sx={{ mb: 1 }}>
          <InputLabel id={`approved-label-${reg.id}`}>Status</InputLabel>
          <Select
            labelId={`approved-label-${reg.id}`}
            value={reg.approved ? "true" : "false"}
            label="Status"
            onChange={(e) => updateRegistrationStatus(reg, e.target.value === "true")}
          >
