...
const EventManagement: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    event_date: "",
    location: "",
    min_tier: 1, // 默认是 Basic (1)
  });

  // 加载活动
  const fetchEvents = () => {
    axios
      .get("http://localhost:8000/zoo/api/events/")
      .then((response) => {
        setEvents(response.data);
      })
      .catch((error) => console.error("Error fetching events:", error));
  ...
  const fetchRegistrations = () => {
    axios
      .get("http://localhost:8000/zoo/api/registrations/")
      .then((response) => {
        setRegistrations(response.data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const eventData = {
        ...formData,
        min_tier: Number(formData.min_tier), // 确保是数字
      };
      ...
      if (selectedEvent) {
        ...
        await axios.post("http://localhost:8000/zoo/api/events/", eventData, {
          withCredentials: true,
        });
        setMessage("Event created.");
      }

      setFormData({ title: "", description: "", event_date: "", location: "", min_tier: 1 });
  ...
  const handleEdit = (event: SpecialEvent) => {
    ...
    setFormData({
      ...
      event_date: event.event_date,
      location: event.location,
      min_tier: event.min_tier, // 这里确保是数字
    });
  };
  ...
  const handleDelete = async (eventId: number) => {
    try {
      await axios.delete(`http://localhost:8000/zoo/api/events/${eventId}/`, {
        withCredentials: true,
      });
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 4, p: 2, border: "1px solid #ccc" }}>
        ...
        <form onSubmit={handleSubmit}>
          <TextField label="Title" name="title" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} fullWidth sx={{ mb: 2 }} required />
          <TextField label="Description" name="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} fullWidth multiline sx={{ mb: 2 }} required />
          <TextField label="Event Date" name="event_date" type="datetime-local" value={formData.event_date ? formData.event_date.slice(0, 16) : ""} onChange={(e) => setFormData({ ...formData, event_date: e.target.value })} fullWidth sx={{ mb: 2 }} required />
          <TextField label="Location" name="location" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} fullWidth sx={{ mb: 2 }} required />

          <FormControl fullWidth sx={{ mb: 2 }}>
            <InputLabel id="min-tier-label">Minimum Tier</InputLabel>
            <Select labelId="min-tier-label" name="min_tier" value={formData.min_tier} label="Minimum Tier" onChange={(e) => setFormData({ ...formData, min_tier: Number(e.target.value) })} required>
      ...
      {events.map(event => (
        <Card key={event.id} sx={{ mb: 2 }}>
          <CardContent>
            <Typography variant="h5">{event.title}</Typography>
            <Typography>Minimum Tier: {TIER_LABELS[event.min_tier]}</Typography>
            <Button variant="outlined" onClick={() => handleEdit(event)} sx={{ mr: 1 }}>Edit</Button>
