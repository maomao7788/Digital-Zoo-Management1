...
const ZookeeperPage: React.FC = () => {
  ...
  const handleUserChange = (e: SelectChangeEvent) => {
    const selectedUserId = e.target.value === "" ? 0 : Number(e.target.value);
  ...
  return (
    <div>
      ...
      <div>
        {/* 选择 User 自动填充 Email */}
        <Select
  value={newZookeeper.user.id.toString()} 
  onChange={handleUserChange} // ✅ 使用正确的类型
  displayEmpty
...
>
  ...
  {users.map((user) => (
    <MenuItem key={user.id} value={user.id}>{user.username}</MenuItem>
        ...
        <TextField label="Email" value={newZookeeper.email} disabled /> {/* ✅ 只读 Email */}
      ...
      <TableContainer component={Paper} sx={{ marginTop: 2 }}>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell>Zookeeper Name</TableCell> {/* ✅ 修改 Username 为 Zookeeper Name */}
          ...
          <TableBody>
            {zookeepers.map(zookeeper => (
              <TableRow key={zookeeper.id}>
                <TableCell>{zookeeper.user.username}</TableCell> {/* ✅ 使用 User.username 作为 Zookeeper Name */}
