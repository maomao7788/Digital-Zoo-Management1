...
const ZookeeperPage: React.FC = () => {
  ...
  const [users, setUsers] = useState<User[]>([]);  // ✅ 维护用户列表
  const [newZookeeper, setNewZookeeper] = useState<Zookeeper>({
    user: { id: 0, username: '', email: '' },  // ✅ 确保 user 具有 id
    role: '',
    qualifications: '',
    responsibilities: '',
    email: '',
  ...
  const loadUsers = async () => {
    try {
      const response = await axios.get('http://localhost:8000/zoo/api/users/');  // ✅ 确保 Django 端点返回用户
  ...
  const handleCreateZookeeper = async () => {
    try {
      ...
      await createZookeeper({
        user: newZookeeper.user.id,  // ✅ 传递 User ID，而不是对象
  ...
  return (
    <div>
      ...
      <div>
        <Select
          ...
          onChange={(e) => {
            ...
            setNewZookeeper((prev) => ({
              ...prev,
              user: selectedUser || { id: 0, username: '', email: '' },
            }));
          }}
          displayEmpty
        ...
        <TextField label="Email" value={newZookeeper.email} onChange={(e) => setNewZookeeper({ ...newZookeeper, email: e.target.value })} />
            ...
            {zookeepers.map(zookeeper => (
              <TableRow key={zookeeper.id}>
                ...
                <TableCell>
                  ...
                  <Button variant="contained" color="error" onClick={() => deleteZookeeper(zookeeper.id!)}>Delete</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
