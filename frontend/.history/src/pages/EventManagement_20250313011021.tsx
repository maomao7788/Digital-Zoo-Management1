...
const EventManagement: React.FC = () => {
  ...
  const updateRegistrationStatus = async (reg: Registration, newStatus: string) => {
    try {
      ...
      fetchRegistrations();
      fetchEvents(); // 更新活动中的 participants 列表
    } catch (error: any) {
  ...
  const updateEvent = async () => {
    ...
    try {
      await axios.put(`http://localhost:8000/zoo/api/events/${selectedEvent.id}/`, editingEvent, { withCredentials: true });
