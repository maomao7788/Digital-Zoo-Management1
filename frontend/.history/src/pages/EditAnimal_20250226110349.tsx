...
const EditAnimal = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    lifespan: 0,
    behaviour: '',
    habitat: '' as string | number, // 允许字符串或数字
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (id) {
      updateAnimal(parseInt(id), { ...formData, habitat: Number(formData.habitat) })
        .then(() => {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <FormControl fullWidth margin="normal">
          ...
          <Select
            ...
            onChange={e => setFormData({ ...formData, habitat: Number(e.target.value) })}
          >
