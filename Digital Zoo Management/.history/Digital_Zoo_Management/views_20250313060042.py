...
class SpecialEventViewSet(viewsets.ModelViewSet):
    ...
    def available(self, request):
        ...
        events = SpecialEvent.objects.filter(min_tier__lte=user_tier)
...
#         valid_status_choices = dict(EventRegistration.STATUS_CHOICES).keys()
...
class EventRegistrationViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        user_membership = getattr(user, 'membership', None)
        if not user_membership:
            return Response({"detail": "You must be a member to register."}, status=status.HTTP_403_FORBIDDEN)
        tier_order = {"Basic": 1, "Premium": 2, "VIP": 3}
        user_tier = user_membership.tier
        event_min_tier = event.min_tier
        if tier_order[user_tier] < tier_order[event_min_tier]:
            return Response(
                {"detail": f"Your membership level ({user_tier}) does not meet the minimum requirement ({event_min_tier})."},
                status=status.HTTP_403_FORBIDDEN
            )
        existing_registration = EventRegistration.objects.filter(event=event, user=user).first()
        if existing_registration:
            return Response({"detail": "You have already registered for this event."}, status=400)
        registration = EventRegistration.objects.create(event=event, user=user)
        return Response({"detail": "Registration submitted. Waiting for approval.", "approved": registration.approved}, status=201)
    ...
    def update_status(self, request, pk=None):
        ...
        registration.approved = not registration.approved
        registration.save()
        if registration.approved:
            event.participants.add(registration.user)
        else:
            event.participants.remove(registration.user)
