...
class ApplyMembershipView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not user.is_authenticated:
            return Response({"error": "You must be logged in to apply for membership."}, status=status.HTTP_401_UNAUTHORIZED)

        tier = request.data.get('tier')
        if not tier:
            return Response({"error": "Membership tier is required."}, status=status.HTTP_400_BAD_REQUEST)
...
class UserMembershipView(generics.RetrieveAPIView):
    ...
    def get_object(self):
        try:
            return self.request.user.membership  # 获取当前用户的会员信息
        except Membership.DoesNotExist:
            raise NotFound({"error": "No membership found for this user."})  # 返回 404 而不是 500
