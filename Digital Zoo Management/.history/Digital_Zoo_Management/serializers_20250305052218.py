...
class ActivityLogSerializer(serializers.ModelSerializer):
    ...
    completion_time = serializers.DateTimeField(format="%Y-%m-%d %H:%M:%S", read_only=True)
    zookeeper_name = serializers.CharField(source='zookeeper.user.username', read_only=True)
    animal = AnimalSerializer(read_only=True)
    animal_id = serializers.PrimaryKeyRelatedField(
        queryset=Animal.objects.all(),
        source='animal',
