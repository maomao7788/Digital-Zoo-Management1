...
class SpecialEventCreateView(generics.CreateAPIView):
    serializer_class = SpecialEventSerializer
    permission_classes = [permissions.IsAuthenticated]
...
class SpecialEventDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = SpecialEvent.objects.all()
    serializer_class = SpecialEventSerializer
    permission_classes = [permissions.IsAuthenticated]  
...
class EventRegistrationUpdateView(generics.UpdateAPIView):
    ...
    serializer_class = EventRegistrationSerializer
    permission_classes = [permissions.IsAuthenticated]  # 可加权限检查
...
class SpecialEventUpdatePostView(APIView):
    permission_classes = [permissions.IsAuthenticated]

    def post(self, request, pk, format=None):
        try:
            event = SpecialEvent.objects.get(pk=pk)
        except SpecialEvent.DoesNotExist:
            return Response({"error": "Event not found"}, status=status.HTTP_404_NOT_FOUND)
    
        serializer = SpecialEventSerializer(event, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
