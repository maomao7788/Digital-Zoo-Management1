...
class SpecialEventViewSet(viewsets.ModelViewSet):
    ...
    def available(self, request):
        ...
        # 定义会员等级的数值映射
        tier_ordering = Case(
            When(min_tier="Basic", then=Value(1)),
            When(min_tier="Premium", then=Value(2)),
            When(min_tier="VIP", then=Value(3)),
            output_field=IntegerField(),
        )

        # 获取用户等级对应的数值
        user_tier_value = {"Basic": 1, "Premium": 2, "VIP": 3}.get(user_tier, 0)

        # 过滤出满足用户等级的活动
        events = SpecialEvent.objects.annotate(
            tier_rank=tier_ordering  # 为每个活动添加 `tier_rank`
        ).filter(tier_rank__lte=user_tier_value)  # 只选 `tier_rank <= user_tier_value` 的活动
