...
class SpecialEventViewSet(viewsets.ModelViewSet):
    ...
    def available(self, request):
        """
        返回当前用户会员等级可见的活动列表
        当前用户的会员等级存放在 request.user.membership.tier 中
        """
        user = request.user
        try:
            membership = user.membership  # OneToOneField 的相关对象
        except Membership.DoesNotExist:
            return Response({"detail": "Membership not found."}, status=400)
        
        user_tier = membership.tier
        # 假设 min_tier 和 membership.tier 的取值均为 'Basic', 'Premium', 'VIP'
        # 且字典序 Basic < Premium < VIP 恰好符合会员级别的排序
