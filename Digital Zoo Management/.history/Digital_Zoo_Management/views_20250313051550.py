...
class EventRegistrationViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # 创建报名，默认 `approved=False`
        registration = EventRegistration.objects.create(event=event, user=user)
        return Response({"detail": "Registration submitted. Waiting for approval.", "approved": registration.approved}, status=status.HTTP_201_CREATED)
    ...
    def update_status(self, request, pk=None):
        ...
        registration.save()
        return Response({"message": "Status updated successfully", "approved": registration.approved}, status=status.HTTP_200_OK)
