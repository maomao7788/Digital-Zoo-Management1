...
class ApplyMembershipView(generics.CreateAPIView):
    # 如果你希望允许匿名访问，可以设置 AllowAny，
    # 但需要在 perform_create 中手动判断是否登录
    permission_classes = [permissions.AllowAny]
    serializer_class = MembershipSerializer

    def perform_create(self, serializer):
        # 模仿 get_user_tasks 中对 request.user.is_authenticated 的判断
        if not self.request.user.is_authenticated:
            # 如果用户未登录，则抛出权限异常
            raise PermissionDenied("Unauthorized: You must be logged in to apply for membership.")
