...
class ApplyMembershipView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if not user.is_authenticated:
            raise PermissionDenied("Unauthorized: Please log in before applying for membership.")
        ...
        membership, created = Membership.objects.update_or_create(
            ...
            defaults={
                ...
                "benefits": benefits,
                "expiry_date": expiry_date,
                "active": False  # 申请后默认未激活，需要管理员审批
            }
        )

        # 如果是更新，而不是新创建的，返回提示
        if not created:
            raise ValidationError("You already have a membership. Please wait for approval or renew later.")
