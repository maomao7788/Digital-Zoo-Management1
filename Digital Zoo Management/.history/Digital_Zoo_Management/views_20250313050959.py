...
class EventRegistrationViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response({"detail": "Registration successful.", "status": registration.status}, status=status.HTTP_201_CREATED)
    ...
    def update_status(self, request, pk=None):
        ...
        # 获取新状态
        new_status = request.data.get('status')
        valid_status_choices = ["Pending", "Approved", "Rejected"]
        ...
        registration.status = new_status
        registration.save()
        return Response({"message": "Status updated successfully", "status": registration.status}, status=status.HTTP_200_OK)
