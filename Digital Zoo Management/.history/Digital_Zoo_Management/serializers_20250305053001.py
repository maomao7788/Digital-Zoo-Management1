...
class CareRoutineSerializer(serializers.ModelSerializer):
    ...
    # zookeeper = ZookeeperSerializer(read_only=True)  
    # zookeeper_id = serializers.PrimaryKeyRelatedField(queryset=Zookeeper.objects.all(), source='zookeeper', write_only=True)

    # animal_info = AnimalSerializer(source='animal', read_only=True) 
    # zookeeper_info = ZookeeperSerializer(source='zookeeper', read_only=True) 
...
class ActivityLogSerializer(serializers.ModelSerializer):
    ...
    animal_info = AnimalSerializer(source='animal', read_only=True) 
    zookeeper_info = ZookeeperSerializer(source='zookeeper', read_only=True)  
    care_routine_info = CareRoutineSerializer(source='care_routine', read_only=True)
